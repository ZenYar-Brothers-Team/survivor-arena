...
Workflow tooling follow-up, 2026-09-22: пользователь разрешил пакет подготовки утверждённого арта, numeric-only visual-preview и единый безопасный runner проверок. Tooling packet: In progress, проверяется реальный Unity runner; обязательные acceptance IP и их gates сохраняются. [DECISION-0049](../decisions/0049-art-workflow-automation-and-preview.md), [команды](../../scripts/README.md). Execution order и Next Ready не изменены.
