...
## Хранение отзыва в проекте

После обработки ручного прогона переносить отзыв в [docs/playtests](../playtests/README.md): один файл на reportId по [шаблону](../playtests/TEMPLATE.md), отдельные OBS-NN внутри. Это основная редактируемая запись после переноса; локальный feedback остаётся исходным материалом. Для диагностики выбранного бага сохранять неизменённый run.json и snapshot исходного feedback рядом в reports/, связывать относительными ссылками и SHA256. Повторный экспорт не меняет Git-запись автоматически. Новые локальные заметки переносить дополнением, сохраняя цитаты и диагностику. Runtime path и schema v1 не меняются.
...
4. Записать observed vs expected и причину окончания в feedback, сообщить путь к пакету. Неизвестное явно отметить. Synthetic smoke не считается ручным прогоном.
5. При обработке сохранить запись в docs/playtests по правилам выше, проверить IDs/hash/quality и добавить ссылку в индекс. Замечание закрывать только после проверки исправления; экспорт сам по себе его не закрывает.
