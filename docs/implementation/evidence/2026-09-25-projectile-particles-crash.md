...
| Проверка в игре (FPS при взрывах сфер) | Пользователь, 2026-09-25: «кажется пофиксилось» — просадка не воспроизводится; численных FPS-замеров нет |
...
Второй прогон: та же проверка, 0 и после шага `Simulate` — в EditMode `particleCount` вручную
симулируемой системы не наблюдаем. Все структурные проверки до неё проходили в обоих прогонах (нет
исключений и ошибок `AddComponent`, две отдельные child-системы, root без `ParticleSystem`). Подсчёт частиц
удалён; вместо него тест проверяет независимые хвосты (impact 0.1 s закончился, explosion 0.2 s ещё идёт,
возврат в пул только после обоих).
