...
3. **Пакет AI.** Сохранить запись прогона и выбранные артефакты в docs/playtests, проверить связанные IDs, hash и quality; сослаться на OBS-NN. Передать report, notes, configs snapshot/hash и только релевантные GDD sections/полные Content cards. Если данные incomplete, AI сохраняет это ограничение в выводах.
...
8. **Повторный ручной тест.** Сопоставимые сценарии и условия; до/после report IDs и config hashes. Новый прогон получает отдельную запись со ссылкой на исходное замечание; в исходной записи сохраняются диагностика, diff/commit и результат повторной проверки. Итог: оставить, откатить, проверить ещё одну гипотезу либо признать данных недостаточно. Совпадающий seed не превращает ручные действия и Unity physics в deterministic replay.
...
- Локальные outputs находятся вне `Assets` и runtime `Resources`; точный пользовательский output path/retention задаётся delivery contract. Raw сессии не коммитятся автоматически; выбранный для диагностики или сравнения run.json сохраняется без изменений в docs/playtests/reports/ после просмотра содержимого. Правила записи, снимка исходного feedback и обработки новых заметок — в docs/playtests/README.md. Ничего не отправляется AI/в сеть автоматически.
