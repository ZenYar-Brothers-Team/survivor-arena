# Обновление существующих IP и производство арта
...
Переработанный проект `design-sync-R2`. **Существующие IP корректируются до следующей реализации.** Полные reviewable replacement packets находятся в [modules](modules/); прежние operative files сейчас не меняются. Здесь нет модели «сначала закончить старый план, потом дописать новые IP».

Все пять новых source documents утверждены. Номера действующих IP сохраняются, а порядок ещё не реализованной работы меняется через отдельную Execution order в STATUS после принятия плана. Подробности статусов и регистрации — [05](05-migration-and-execution.md).

## Общий контракт

- Каждый linked packet является полной целевой спецификацией. Старый файл — implementation/evidence baseline, не второй конкурирующий Scope. Реализацию переиспользуем; новые требования и проверки добавляем только там, где есть delta.
...
- После реализации owning specification/evidence в STATUS и готовность потребителей синхронизируются. Existing historical Verified не является target verification; точный переход описан в [05](05-migration-and-execution.md).
...
- Только world pickups, Traveler framework/content и локальный playtest/balance процесс получили пять новых IDs [IP-28…32](02-new-modules.md).
...
Применяются .claude gameplay/content/UI/foundation/Unity-test rules и helpers design-review, consistency-check, content-audit, asset-audit, perf-audit, test-quality-review, smoke-check. Hook проверяет только JSON syntax в своём клиенте, не domain/ref validity. Код, source docs, .claude и actual STATUS этим proposal не меняются.
