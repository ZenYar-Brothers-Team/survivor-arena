# Сопоставление утверждённого target design с текущей реализацией
...
Дата: 2026-09-20. Этот файл — часть проекта Implementation Plan для ревью; он не меняет исходные документы, код или execution status.

## 1. Уточнённая исходная договорённость

Пользователь явно подтвердил: **все пять новых документов уже утверждены**. Game Design v2, Content Design v2 и Art Direction v2 впоследствии должны **полностью заменить** соответствующие старые документы. Последняя поправка пользователя: старые три файла заменяются новыми без отдельных архивных копий; сравнение ниже относится к исходной базе аудита. Поэтому ниже новые документы — утверждённый целевой дизайн, а прежние канонические файлы и репозиторий — база текущей реализации. Повторное одобрение уже описанных правил, карточек или общего художественного направления не требуется.

Утверждение документа не создаёт отсутствующее число, не разрешает выбрать произвольный вариант внутреннего противоречия и не означает, что будущая ещё не созданная картинка принята. Три разных факта учитываются отдельно: **design approved**, **implementation data complete**, **implemented/verified**. Первый подтверждён пользователем; второй проверяется по использованным полям конкретной карточки; третий остаётся в `STATUS.md`. Настоящий проект также ещё не регистрирует новые IP в исполнение.

Источники из коммита `a2dd22a8eb61fb851610a94e09533c017277b287` автора `zhenia966` (`new docs`, 2026-09-19):

| Утверждённый target | Действующая база / действие последующей миграции |
|---|---|
| [Game Design v2](../../../Game%20Design%20v2.md) | Полная замена `docs/Game_design.md` |
| [Content Design v2](../../../Content%20Design%20v2.md) | Полная замена `docs/Content_design.md` |
| [Art Direction v2](../../../art/Art%20Direction%20v2.md) | Полная замена `docs/art/ART_DIRECTION.md` |
...
Указанные ниже номера строк относятся к исходным файлам до нормализации Markdown. После миграции Context в IP должен ссылаться на стабильные разделы и content IDs, а не на старую нумерацию строк.
...
|---|---|---|
| G-01 Empty/short draft | Три слота утверждены; пропущена старая empty rule. Предложение delta: сохранить earned level и skip пустого ordinary draft без зависшей паузы; отдельно описать 1–2 eligible и Book-empty | M-01/IP-07/IP-10/IP-11; approved target amendment и тесты 0/1/2 options, only-sets failed roll |
...
| G-04 SET-002 | `Content Design v2.md:393–400` усиливает return disc, но SKILL-008:166–174 не содержит return phase | M-01/IP-11/IP-08; решить только эту недостающую семантику до реализации затронутого set |
| G-05 SET-015 / trash | SET-015:510–518 бафает explosion у SKILL-016, у которого base explosion нет. SET-008 добавляет его, но set-to-set amplification по умолчанию запрещено | M-01/IP-11; applicability без непредусмотренной proc chain |
...
| G-14 Encounter/content numbers | ENEMY contact intervals; boss speed/attack/reward details; traveler scaling/presence/XP; numeric character weights; field pools/geometry/schedules отсутствуют или неполны | IP-20…24/IP-30/IP-32; per-ID/per-field completeness, отдельные production schedule IDs |
| G-15 Meta and run exit | Base meta design и immediate Retry утверждены; quit/reward/failure semantics, «завершить FIELD» win vs any result, prices/rewards/save idempotency нуждаются в конкретизации | IP-25/IP-26; no duplicate reward/retry double-start и terminal precedence |
| G-16 Presentation/settings | Screen shake toggle утверждён; camera visual offset vs centered-follow, settings persistence/apply/failure, optional audio routing требуют контракта. Boss UI относится к final boss | IP-26/IP-15/IP-10A; follow center сохранён, toggle выключает offset, owner scopes явные |
...
| G-18 Player palette | Art Direction v2 §4.4 требует всегда зелёного игрока, §9.2 допускает другие palettes для future species | M-01/IP-12A; явная согласованная поправка формулировки, не повторный approval негоблинского roster |
...
## 8. Последующая полная замена и синхронизация — M-01

1. Зафиксировать источники пяти утверждённых target файлов. Полностью заменить три canonical bodies без архивных копий, backup или отдельного snapshot прежних документов.
2. При реализации M-01 выполнить **полную замену** трёх canonical destinations содержимым трёх target документов. Это не выборочный merge понравившихся изменений и не сохранение старого Draft-каталога как активного источника.
3. Нормализовать escaped Markdown (`\#`, backticks, links, `\<br\>`) без смены смысла. Любой substantive gap resolution из §6 оформить отдельным явным delta/decision, чтобы format cleanup не скрывал новый product rule.
4. Сохранить историю DECISION-0001…0014 и результатов Verified IP. Устаревающее описание реализации дополнить новым решением/ссылкой; не переписывать прошлое evidence так, будто новое поведение уже проверено.
5. Зафиксировать semantic migrations ID: SKILL-001, PASSIVE-002/007, SET-001…008 и renamed characters. Имена сами по себе не требуют новых ID; изменённый смысл требует корректной исторической атрибуции в saves/telemetry/balance reports.
6. Синхронно обновить active links, `Implementation_plan.md`, README/WORKFLOW и selector в AGENTS.md, все затронутые IP Context/Scope/dependencies и `STATUS.md`. Зарегистрировать новые IP только после review этого плана. Dependency edges указываются с обеих сторон. WORKFLOW выбирает первый Ready из новой Execution order в STATUS вместо численной сортировки ID; target scope revision связывается с verification evidence по правилам [05](05-migration-and-execution.md).
7. Переформулировать прежний общий CG-01: утверждение нового каталога уже получено; remaining blocker — конкретные отсутствующие data/semantics/assets, а не Draft-статус старой версии. CG-02 schedules, CG-03 economy, CG-04 exact final timing остаются по фактической незаполненности, пока их данные не предоставлены.
8. В `.claude` учесть explicit user approval, который имеет приоритет над «ignore v2 unless named» и над требованием повторного review Draft-карточек. Устаревшие ссылки обновлять вместе с миграцией только в пределах принятого scope; не менять read-only skills или automation hooks молча. Asset approval остаётся отдельным конкретным шагом pipeline.

## 9. Проверка покрытия и границы

Эта сверка покрывает обе версии Game/Content, различия Art Direction, новый UX и Art Production, действующие решения и релевантные runtime границы. Проверены все группы 121 target IDs, но не изобретались production schedules, мета-экономика, отсутствующие значения и изображения. Unity tests здесь не запускались: это проект документации, не утверждение о новом runtime verification.

Полные целевые спецификации всех 35 модулей — [modules](modules/), только пять новых scopes — [02](02-new-modules.md), карта обновления существующих IP и поставка assets — [03](03-existing-modules-and-art.md), простой ручной цикл баланса — [04-balance-loop.md](04-balance-loop.md). Результаты этой сверки являются входом для плана, а не самостоятельным execution-status registry.
