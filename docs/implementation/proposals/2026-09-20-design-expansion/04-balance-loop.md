...
Проект контрактов IP-31/IP-32. Основание — отдельное пожелание пользователя: **ручные прогоны → комментарии и логи для AI → предложение корректировок → approval → изменения**. Пять новых документов уже утверждены; они не содержат законченного run-report/AI-review процесса. Этот файл предлагает процесс, а не новые балансные числа или автоматизацию, которая сама их применяет.
...
- `.claude` JSON hook проверяет синтаксис и работает в своём клиенте. Нужны отдельные catalog/domain/reference checks; в этом draft никакие тесты Unity не запускались.
