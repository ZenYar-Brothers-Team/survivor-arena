...
Показывать только значимые отклонения от baseline; полный набор точных внутренних характеристик на этом экране не нужен. Например:
...
\#\#\# Reroll  
Кнопка заменяет все текущие предложения по правилам GDD.
...
Set-information на карточках работает так же, как в обычном draft.

Если Book может предлагать ограниченный специальный pool, карточки всё равно используют тот же reusable UI component.
...
После короткой паузы / подтверждения открывается Run Results.

\---

\# 16\. Run Results

Один простой summary screen.
...
Permanent upgrades могут отображаться обычным списком/сеткой карточек.
...
2\. На Character Select показываются только значимые modifiers относительно baseline, без полного списка точных внутренних stats.  
