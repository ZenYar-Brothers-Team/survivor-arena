...
For a named entity/system, search its IDs and owning IP first; widen to all prefixes only for `full` or when a cross-reference requires it. Grep IDs by prefix (`CHAR-`, `PASSIVE-`, `SKILL-`, `ENEMY-`, `BOSS-`, `SET-`, plus any prefixes present) in `docs/Content_design.md`, `docs/Game_design.md`, `docs/Implementation_plan.md`, `docs/implementation/**`, `docs/decisions/**`. Exclude `docs/implementation/evidence/**` and `docs/implementation/proposals/**` from current-canon comparisons; consult a specific record only for provenance or an explicit historical comparison. Record for each ID: where defined (full card), where referenced.
...
- **Status text vs repository**: `STATUS.md` claims (Verified/Implemented, test counts, evidence paths) that contradict the repository; inspect the affected IP's linked record when needed. Historical scope statuses are dated evidence, not current execution status; local `TestResults` paths are not committed artifacts.
