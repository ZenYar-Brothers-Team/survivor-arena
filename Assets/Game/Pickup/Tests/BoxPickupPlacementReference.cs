...
{
    /// <summary>
    /// Verbatim copy of the original per-call grid search that <see cref="BoxPickupPlacement.TryPlace"/> replaced
    /// (rebuilt every coordinate list, visited array and queue per call). Test oracle only: the optimized search
    /// must return exactly the same point, including equal-distance ties.
    /// </summary>
    internal sealed class BoxPickupPlacementReference
    {
        private readonly Rect _bounds;
        private readonly Rect[] _obstacles;
        private readonly Vector2 _anchor;

        public BoxPickupPlacementReference(Rect bounds, IEnumerable<Rect> obstacles, Vector2 playerHalfSize, Vector2 anchor, float skin)
        {
        ...
        private bool ClearSegment(Vector2 a, Vector2 b)
        {
            foreach (var rect in _obstacles)
                if (a.x == b.x ? a.x > rect.xMin && a.x < rect.xMax && System.Math.Max(a.y, b.y) > rect.yMin && System.Math.Min(a.y, b.y) < rect.yMax
                    : a.y > rect.yMin && a.y < rect.yMax && System.Math.Max(a.x, b.x) > rect.xMin && System.Math.Min(a.x, b.x) < rect.xMax) return false;
            return true;
        }
