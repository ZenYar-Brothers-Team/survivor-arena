...
namespace Game.Progression.Tests
{
    public sealed class DraftBackfillTests
    {
        ...
        public void OneEmptySlot_TwoFailedSets_UseEqualHalfIntervals(float roll, string expected)
        {
            ...
            var pool = new DraftPool(new BuildEntryDefinition[] { active, passive, a, b }, setOffers: new FixtureSetDraftOfferProvider(0f));
            var result = pool.CreateOptions(SetTestData.Build(active), 3, new FixedDraftRandom(roll));
        ...
        public void OnlySetsFailedChance_FillWithoutDuplicates_AndRespectBanishAndAcquired()
        {
            ...
            var pool = new DraftPool(new BuildEntryDefinition[] { active, a, b }, setOffers: new FixtureSetDraftOfferProvider(0f));
        ...
        public void FullOrdinaryPool_FailedSetsDoNotDisplaceItsCards()
        {
            ...
            var pool = new DraftPool(new BuildEntryDefinition[] { active,
                ...
                new BuildEntryDefinition("FIXTURE-P2", BuildEntryKind.PassiveItem, "P2"), Set("FIXTURE-SET", active) }, setOffers: new FixtureSetDraftOfferProvider(0f));
            var options = pool.CreateOptions(SetTestData.Build(active), 3, new FixedDraftRandom(0.9f));
        ...
        {
            var active = new BuildEntryDefinition("FIXTURE-ACTIVE", BuildEntryKind.ActiveSkill, "Skill");
            var a = SetTestData.Define("FIXTURE-SET-A", "A",
                new SetRecipeComponent(active.Id, BuildEntryKind.ActiveSkill, 1));
            var b = SetTestData.Define("FIXTURE-SET-B", "B",
                new SetRecipeComponent(active.Id, BuildEntryKind.ActiveSkill, 1));
            var pool = new DraftPool(new BuildEntryDefinition[] { active, b, a,
                new BuildEntryDefinition("FIXTURE-P1", BuildEntryKind.PassiveItem, "P1"),
                new BuildEntryDefinition("FIXTURE-P2", BuildEntryKind.PassiveItem, "P2") }, setOffers: new FixtureSetDraftOfferProvider(0.5f));
        ...
        private static SetDefinition Set(string id, BuildEntryDefinition active) => SetTestData.Define(
            id, id, new SetRecipeComponent(active.Id, BuildEntryKind.ActiveSkill, 1));
    }
}
